...
export default {
  ...
  methods: {
    ...
    editRow: function(row) {
      this.$axios
        .get("/exercise/getSitupById?id=" + row.id)
        .then(situp => {
          // 展示对话框
          this.dialogModel = {
            title: "编辑俯卧撑",
    ...
    submitForm() {
      this.$refs["situp"].validate(valid => {
        if (valid) {
          this.$axios
            .post("/exercise/saveSitup", this.dialogModel.form)
            .then(() => {
